...
foreach ($target in $buildTargets) {
    Write-Host "  $($target.Architecture): $($target.BinFolder)  app: $($target.AppBundlePath)"
}
...
function New-ReleaseAssets {
    ...
    foreach ($target in $buildTargets) {
        ...
        if (-not (Test-Path -LiteralPath $appBundle)) { throw "Missing app bundle (run build.ps1 first): $appBundle" }
        Compress-Archive -Path $appBundle -DestinationPath $portable -Force
        $paths.Add($portable)
    }
    return @($paths)
}
...
try {
    ...
    Write-Host "`nStaging macOS portable assets..."
